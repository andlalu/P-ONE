...
\date{\item First version: June 16, 2018 \item Previous Version : March 1, 2026 \item This version: \today}
%start file content
\begin{document}
\maketitle
%\abstract{
...
$P(X_i,T,K;\theta^{\mathbb{P}\cap\mathbb{Q}};\theta^\mathbb{Q})$. Let $\sigma_i^{0} = \left(\sigma_i^{(1),0},\dots,\sigma_i^{(q_i),0}\right)^\top$ denote the corresponding Black-Scholes implied volatility vector and let $ \mathcal{V}_i = \left(\mathcal{V}_i^{(1)},\dots,\mathcal{V}_i^{(q_i)}\right)^\top$ denote the associated Black-Scholes vega vector.
...
\paragraph{Cross-sectionally correlated noise}
...
\paragraph{Temporally persistent noise}
...
\]
The raw implied-volatility perturbation is
\begin{align}
\tilde\sigma_i^{(j),C}
=
\max\left\{
...
\end{align}
where $s_{\sigma,u,i}^{(j)}$ is allowed to depend on $m_i^{(j)}$ and $\tau_i^{(j)}$. The factors in \eqref{eq:factor_noise} are intended to represent a persistent common component in the observation error, affecting the level, moneyness slope and maturity slope of the contaminated implied-volatility panel. This provides a parsimonious route to temporal dependence without forcing a full scale spatio-temporal observation error specification.
