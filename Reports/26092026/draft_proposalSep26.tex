...
\date{\item First version: June 16, 2018 \item Previous Version: August 6, 2026 \item This version: September 25, 2026}
%start file content
\begin{document}
\maketitle
%\abstract{
...
\section{Assumed Data Generating Process And Option Pricing}
...
\begin{align}
	...
	\dif{V_t} &= \kappa(\bar{V}-V_t)\dif t + \sigma_V\sqrt{V_t}\left(\rho \dif W_{t,1}^\mathbb{P} + \sqrt{1-\rho^2}\dif W_{t,2}^\mathbb{P}\right). \label{PHestonV}
\end{align}
...
\begin{align}
	...
	\dif{V_t} &= \kappa(\bar{V}-V_t)\dif t + \eta_{V}V_t\dif t + \sigma_V\sqrt{V_t}\left(\rho \dif W_{t,1}^\mathbb{Q} + \sqrt{1-\rho^2}\dif W_{t,2}^\mathbb{Q}\right).\label{QHestonV}
\end{align}
...
This difference can be, inter alia, attributed to microstructure effects or stale quotes. \citet{duarte2024very}
suggests that such frictions can be quantitatively important for inference involving option-implied risk premia. 

The proposed simulation design therefore does not commit ex-ante to a single parametric error law but instead considers four observed-panel specifications generated from the same clean model-implied panel. Each specification is meant to reflect empirical concerns raised by the recent literature while keeping the data generating exercise tractable. The four specifications are introduced in implied-volatility space. The corresponding noisy prices are then obtained through the Black-Scholes pricing formula and projected back into the no-arbitrage price interval. This keeps the treatment of all four observation error specifications internally consistent while preserving both price and implied-volatility representations of the noisy option panel.
...
The third observation error design allows for a persistent common component in the observation error, affecting the level, moneyness slope and maturity slope of the noisy implied volatility panel. This is consistent with the practical treatment of option panels as surfaces and with the broader evidence that option price noise can be quantitatively relevant for inference on risk premia as in \cite{duarte2024very}.
...
\paragraph{Design D: Variance dependent noise}
The fourth observation error design extends Design C by allowing part of the temporally persistent common observation error to depend directly on the latent variance state. The purpose of this specification is to examine whether the implied-state estimator is more sensitive to observation errors which move systematically with the state being recovered from the option panel, while keeping the unconditional magnitude of the observation error comparable to that of Designs A--C.
...
s_V^2
=
\frac{\bar V\sigma_V^2}{2\kappa},
\]
denote the standardised latent variance state under the stationary distribution of the variance process. Design D adds to the factor specification of Design C a fourth, variance linked common component,
\[
f_{V,i}
=
\gamma_V z_{V,i},
\]
where $\gamma_V$ determines the magnitude of the dependence between the option observation error and the latent variance state.

The noisy implied volatility is then generated as
\begin{align}
\tilde\sigma_i^{(j),D}
=
\max\left\{
...
u_i^{(j),D}
\right\}.
\label{eq:state_dependent_iv_noise}
\end{align}

Thus, Designs C and D share the same persistent level, moneyness-slope and maturity-slope factors, while Design D additionally allows a common component of the observation error to vary with the contemporaneous latent variance state. The residual idiosyncratic scale is adjusted so that the unconditional pointwise implied-volatility observation error remains comparable to that of the other designs. Design D therefore changes the dependence structure of the observation error without increasing its overall magnitude.
...
The option pricing step uses a maturity-specific fixed effective COS interval. The same interval is used when generating the option panels and when reconstructing the implied variance states, while the number of COS terms may differ between these two steps if this is supported by the numerical validation. The calibration of the fixed intervals and the numerical implementation of the state inversion are reported in Appendix~\ref{app:is_cgmm_implementation}.
...
\right]
\]
denote the conditional characteristic function of the next estimation state, where \(s\in\mathbb R^2\). For the Heston model this conditional characteristic function is available in affine form under \(\mathbb P\).
...
\end{equation}
The numerical evaluation of \eqref{eq:is_cgmm_estimator}, including the construction of the implied variance states, the conditional characteristic function and the integration rule for the continuum of moments, is described in Appendix~\ref{app:is_cgmm_implementation}.

The distinction between the clean and noisy option panels is particularly relevant for this estimator. In Design A, independent contract-level observation errors perturb the implied variance state locally through the cross-sectional inversion in \eqref{eq:is_implied_state}. In Design B, cross-sectional dependence in the observation errors can generate coherent distortions across nearby moneyness and maturity points, which may be partially absorbed as changes in the implied state. In Design C, persistent common factors in the option observation error may generate serially dependent distortions in the recovered implied variance sequence. Design D additionally allows the common level component of the observation error to depend on the latent variance state itself, providing a stress test in which part of the measurement error can move systematically with the state being recovered. The Monte Carlo comparison therefore evaluates sensitivity not only to the magnitude of option observation error, but also to its cross-sectional, temporal and state-dependent structure.
...
At the current stage, the Heston simulation engine, the option-panel generation routines and the first-step implied-state C-GMM estimator have been implemented in Python. The estimator constructs scalar date-by-date implied-variance states from option-implied volatilities, evaluates the analytic Heston $\mathbb{P}$-transition characteristic function and forms a first-step C-GMM objective with analytically integrated Gaussian instruments. A 32-step Runge--Kutta implementation is used as a numerical check. For each of the 100 samples, the Monte Carlo exercise uses at most three deterministic starting values and a bounded derivative-free two-pass search under the clean panel and the three observation-error designs. The resulting parameter, implied-state and risk-premium accuracy results are reported in the next section.

\section{Monte Carlo Results for the Implied-State Estimator}
\label{sec:is_cgmm_results}

\subsection{Monte Carlo design and reporting conventions}

The Monte Carlo exercise consists of 100 independent samples and four option-panel designs: the clean benchmark, low i.i.d. observation error, spatially correlated observation error and persistent-factor observation error. Each sample contains 526 weekly state observations and 15 option contracts per date. After constructing the estimation state, the 525 physical return intervals provide 524 conditional residual transitions for the first-step C-GMM criterion. Within each sample, the same simulated path is used for all four designs.

A sample-design estimate is included when its record, simulated path and option panel are complete, the stored files agree with their recorded checksums and all required parameter, implied-state and criterion values are finite. On this basis, all 100 replications are included for each design. The accuracy statistics are first computed within each replication and are then summarised across replications. Throughout the reported accuracy tables, MAE denotes mean absolute error. The parameter table follows the summaries used by \citet{boswijk2024estimating} and also reports bias and RMSE. The results concern the accuracy of the point estimates; standard errors and confidence intervals are not considered.

\subsection{Structural-parameter estimates}

\input{generated/structural_parameter_table.tex}

For the clean panel, the estimates of $\kappa$ and $\bar V$ are close to their true values on average, while the estimates of $\eta$ show more variation. Relative to the clean benchmark, the three observation-error designs also move the mean estimate of $\bar V$ below its true value and increase the RMSE of $\sigma_V$.

\subsection{Accuracy of implied variance states}

For each replication, the final implied variance sequence is matched by date to the simulated variance sequence. Table~\ref{tab:mc_state_accuracy} reports the average across replications of the five accuracy statistics first calculated within each sample. Figure~\ref{fig:mc_state_rmse} shows the distribution of the corresponding RMSE values across samples.

\input{generated/implied_state_table.tex}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{generated/implied_state_rmse_boxplot.pdf}
    \caption{Distribution of implied variance RMSE across the 100 samples. The boxes show the interquartile range, the centre lines show the medians and the whiskers show the 10th and 90th percentiles. Variance errors are multiplied by 100.}
    \label{fig:mc_state_rmse}
\end{figure}

The clean benchmark has the smallest average RMSE. Observation error reduces the accuracy of the implied variance states, but the average errors are similar for the three observation-error designs.

\subsection{Accuracy of risk-premium estimates}

For each replication, the one-month equity and variance risk-premium paths are computed using the definitions in Appendix~\ref{app:risk_premia}. The transformations are applied separately to the parameter estimates and implied variance path from that replication. The corresponding true path is computed using the simulated variance and the true parameters. An error is defined as the estimated value minus the true value. The variance risk premium follows the physical-minus-risk-neutral convention in \citet{boswijk2015asset}.

\input{generated/risk_premium_table.tex}

The average ERP errors are small relative to their variation across replications, although the ERP RMSE remains sizeable. The average VRP error is negative for the clean benchmark and positive for each of the three observation-error designs.

\subsection{Summary of results}

For the clean benchmark, the implied-state estimator recovers the weekly variance path closely. Under the three observation-error designs, observation error approximately doubles the average RMSE of the implied states and somewhat reduces the accuracy of the parameter and risk-premium estimates.
...
We simulate $V_{i+1}$ using the quadratic-exponential scheme of \citet{andersen2008simple} and then update $y_{i+1}$ using a discretisation that preserves the leverage channel implied by $\rho$.

\subsubsection{Quadratic-exponential draw for $V_{i+1}\mid V_i$}
Let $e_i=\exp(-\kappa\Delta)$. Conditional on $V_i$, the first two moments of the square-root variance transition are
\begin{align}
m_i &= \bar V+(V_i-\bar V)e_i, \\
s_i^2 &= \frac{V_i\sigma_V^2e_i(1-e_i)}{\kappa}
+\frac{\bar V\sigma_V^2(1-e_i)^2}{2\kappa},
\qquad
\psi_i=\frac{s_i^2}{m_i^2}.
\label{eq:qe_moments}
\end{align}
The production implementation uses the branch threshold $\psi_c=1.5$. When $\psi_i\leq\psi_c$, define
\begin{align}
b_i^2&=\frac{2}{\psi_i}-1+\sqrt{\frac{2}{\psi_i}}\sqrt{\frac{2}{\psi_i}-1},
&a_i&=\frac{m_i}{1+b_i^2},
\end{align}
and draw $V_{i+1}=a_i(\sqrt{b_i^2}+Z_i)^2$ with $Z_i\sim\mathcal N(0,1)$. When $\psi_i>\psi_c$, set
\begin{align}
p_i=\frac{\psi_i-1}{\psi_i+1},
\qquad
\beta_i=\frac{1-p_i}{m_i}.
\end{align}
For $U_i\sim\mathcal U(0,1)$, the draw is zero if $U_i\leq p_i$ and is
\begin{align}
V_{i+1}=-\frac{1}{\beta_i}\log\left(\frac{U_i-p_i}{1-p_i}\right)
\end{align}
otherwise. Both branches preserve non-negativity and match the conditional moments used by the scheme.
...
&\quad + \sqrt{1-\rho^2}\sqrt{\widehat{I}_i}\,Z_{i+1}.
...
\]
Clean option prices are computed using the COS method. The corresponding Black-Scholes implied volatilities are computed using the same implied-volatility inversion routine used for noisy panels; see \citep{jaeckel2015lets}.

\section{Option Panel Observation Error Specifications}
\label{app:option_noise_specs}

This appendix details the functional forms used to generate noisy option panels from the clean model-implied option panel. The purpose is to introduce observation error structures which are simple enough to be implemented in a large-scale Monte Carlo design, but sufficiently close to observed option-market features to provide a meaningful stress-test of PF and IS estimation.

Noisy option panels are constructed from the clean model-implied panels generated in Appendix~\ref{app:sim_heston} using the observation error designs introduced in the main text. The clean panel at date \(t_i\) consists of model prices \(p_i^0\), Black-Scholes implied volatilities \(\sigma_i^0\), contract characteristics and Black-Scholes vegas. The observation-error functional is applied to the clean implied-volatility value before the corresponding raw noisy price is obtained from the Black-Scholes pricing formula. No-arbitrage projection is then applied to the price when required, followed by implied-volatility inversion to recover the observed noisy implied volatility, using the same procedure for all four designs.


\subsection{Price validity and implied-volatility inversion}
...
\]
The observed price is obtained by projecting the raw noisy price into the admissible interval:
\begin{align}
p_i^{(j),\mathrm{obs}}
=
\min\left\{
...
\end{align}
The projection in \eqref{eq:app_price_projection} is used instead of the theoretically possible alternative of re-sampling the random quantities in the observation-error mechanism. This keeps the distributional
...
the raw noisy price before validity enforcement, the capped observed price, the observed implied
volatility, the noise scenario and indicators for whether lower- or upper-bound capping was applied.
...
\]
Thus, the common component introduces temporal dependence and common variation across moneyness and maturity, while the total marginal implied-volatility observation error remains comparable to that of Designs A and B. This makes the comparison across the four designs depend mainly on the dependence structure of the observation error rather than on a mechanical increase in its absolute scale.

For Design D, the parameters governing the persistent-factor specification are kept equal to those used for Design C. The additional variance-linked component is specified as
\[
f_{V,i}=\gamma_V z_{V,i},
\]
where $z_{V,i}$ is the standardised latent variance state. The parameter $\gamma_V$ controls the magnitude of the state-dependent component. Its numerical value is chosen to generate mild dependence between option observation errors and the latent variance state. The idiosyncratic residual variance is reduced accordingly so that the unconditional pointwise error scale remains comparable to that of Designs A--C.

\subsection{Illustration of the Observation Error Specifications}
\label{app:noise_visualisations}

This subsection reports results based on all 100 samples. Signed implied-volatility error is defined as observed minus model implied volatility. The surface results retain the common moneyness and maturity points. The two dependence measures in Figure~\ref{fig:app_noise_dependence} are calculated separately within each sample, so the boxes show how they vary across the 100 samples.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{generated/noise_dependence.pdf}
    \caption{Dependence in the observation errors across the 100 samples. For each sample, the left panel averages the pairwise correlations between the error series for the 15 contracts. The right panel reports the first-order autocorrelation of the error series for the at-the-money three-month contract. Each box summarises the resulting sample-level values; the centre line is the median and the whiskers show the 10th and 90th percentiles.}
    \label{fig:app_noise_dependence}
\end{figure}

\input{generated/noise_summary_table.tex}

\section{Risk-Premium Definitions}
\label{app:risk_premia}

This appendix gives the definitions of the one-month equity and variance risk premia used in Section~\ref{sec:is_cgmm_results}. They follow the pure-diffusion component in \citet{boswijk2015asset}. For a horizon $h=T-t$, write the expected average variance under $\mathbb P$, conditional on $V_t=v$, as
\begin{align}
\operatorname{AV}^{\mathbb P}(v,h)
&=
\frac{1}{h}\mathbb E_{\mathbb P}\left[\left.\int_t^T V_s\,\dif s\,\right|V_t=v\right]
=\bar V+(v-\bar V)\frac{1-\exp(-\kappa h)}{\kappa h}.
\label{eq:app_average_variance_p}
\end{align}
Under $\mathbb Q$, define
\begin{align}
\kappa_{\mathbb Q}=\kappa-\eta_V,
\qquad
\bar V_{\mathbb Q}=\frac{\kappa\bar V}{\kappa_{\mathbb Q}}.
\end{align}
The corresponding conditional expected average variance is
\begin{align}
\operatorname{AV}^{\mathbb Q}(v,h)
=\bar V_{\mathbb Q}+(v-\bar V_{\mathbb Q})
\frac{1-\exp(-\kappa_{\mathbb Q}h)}{\kappa_{\mathbb Q}h}.
\label{eq:app_average_variance_q}
\end{align}
For the Heston specification in Section~1, the horizon-$h$ equity risk premium and the physical-minus-risk-neutral variance risk premium are therefore
\begin{align}
\operatorname{ERP}_t(h)&=\eta\operatorname{AV}^{\mathbb P}(V_t,h),\\
\operatorname{VRP}_t(h)&=\operatorname{AV}^{\mathbb P}(V_t,h)-\operatorname{AV}^{\mathbb Q}(V_t,h).
\label{eq:app_risk_premia}
\end{align}
The reported horizon is $h=1/12$. ERP is annualised and multiplied by 100 to obtain percentage points. VRP is annualised variance and is also multiplied by 100. For each replication, the estimated paths are calculated from its parameter estimate and final implied state sequence, while the true paths are calculated from its simulated variance sequence and the true parameter vector. The transformations in \eqref{eq:app_average_variance_p}--\eqref{eq:app_risk_premia} are therefore carried out before the results are summarised across replications.
...
\subsection{Fixed COS basis}

For an option maturity \(\mathcal T\), the COS frequencies are fixed as
\begin{equation}
u_k(\mathcal T)
=
\frac{k\pi}{2L_{\mathcal T}},
\qquad k=0,\ldots,N-1,
\label{eq:app_fixed_cos_grid}
\end{equation}
where \([-L_{\mathcal T},L_{\mathcal T}]\) is the effective interval for log-forward moneyness. The interval does not depend on the candidate variance value. The payoff expansion and the affine coefficients \(A_k(\mathcal T)\) and \(B_k(\mathcal T)\) are therefore prepared once for each maturity and risk-neutral parameter vector and reused across strikes and dates.

The variance domain is \(V\in[10^{-6},0.15]\). The parameter ranges are \(\kappa_{\mathbb Q}\in[1.5,4.0]\), \(\bar V_{\mathbb Q}\in[0.012,0.24]\), \(\sigma_V\in[0.20,0.65]\) and \(\rho\in[-0.80,-0.20]\). The calibration uses a seeded Latin-hypercube design together with the true DGP, the initial parameter value and two moderate perturbations. A Carr--Madan Fourier inversion is used to validate an 8192-term COS reference. Table~\ref{tab:app_fixed_cos_calibration} reports the selected specification. Prices with Black-Scholes vega below \(5\times10^{-6}\) are reported separately because numerical price differences close to \(10^{-10}\) produce unstable implied-volatility differences. There are no invalid prices or implied-volatility inversions in the calibration design.

\begin{table}[H]
\centering
\begin{tabular}{lrrrr}
\toprule
\(\mathcal T\) & \(L_{\mathcal T}\) & \(N\) & Max. IV error & Max. IV error, vega screen \\
\midrule
\(1/12\) & 0.75 & 576 & \(2.23\times10^{-4}\) & \(1.81\times10^{-6}\) \\
\(1/4\)  & 1.25 & 576 & \(2.82\times10^{-7}\) & \(2.82\times10^{-7}\) \\
\(1/2\)  & 2.00 & 576 & \(2.77\times10^{-8}\) & \(2.77\times10^{-8}\) \\
\bottomrule
\end{tabular}
\caption{Selected fixed COS basis. The generation and estimation term counts are both 576. The selection uses the lowest term count and then the narrowest interval satisfying the zero-failure error conditions.}
\label{tab:app_fixed_cos_calibration}
\end{table}
...
\]
The implementation evaluates this system using the analytic Heston Riccati solution.
...
\pi_u(u)\,\dif u ,
\]
which reduce to a Gaussian kernel in the distance between the two implied-state observations. This leaves a finite dimensional numerical integration over the \(s\)-argument of the conditional characteristic function. The production implementation uses two-dimensional Gauss--Hermite quadrature of order 4 with scale vector $(1.0,4.0)$ for this remaining integral. Integration over the instrument component remains analytic.
...
\subsection{Production computing arrangement}

The completed Monte Carlo exercise consists of 100 independent samples, four panel designs and 400 estimation tasks. It was carried out on an Amazon EC2 \texttt{c7i.8xlarge} instance with 32 vCPUs and 64 GiB of memory. Twenty-eight samples were processed in parallel. Each process handled one sample at a time, while each numerical library was restricted to one thread. The calculation could be resumed separately by sample and scenario. Persistent EBS storage was used during the calculation and completed results were copied to private S3 storage. These computing arrangements do not affect the numerical specification, which is the same for every task included in the results.
