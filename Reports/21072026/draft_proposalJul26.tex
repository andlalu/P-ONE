...
\subsection{Variance derivative and state solver}

Let
\[
J_i(v;\theta_{\mathbb Q})
=
\frac{\partial\sigma_i^{\mathrm{model}}(v;\theta_{\mathbb Q})}{\partial v}.
\]
Since the COS basis is fixed, the affine contribution can be differentiated without rebuilding the pricing grid:
\begin{equation}
\frac{\partial}{\partial v}\exp(A_k+B_kv)
=
B_k\exp(A_k+B_kv).
\label{eq:app_cos_variance_derivative}
\end{equation}
For an interior implied-volatility solution with sufficient Black-Scholes vega, this gives
\begin{equation}
\frac{\partial\sigma_i^{(j),\mathrm{model}}}{\partial v}
=
\frac{\partial P_i^{(j),\mathrm{model}}/\partial v}
{\operatorname{Vega}_i^{(j)}}.
\label{eq:app_iv_variance_derivative}
\end{equation}
Writing \(L_i(v)=[\sigma_i^{\mathrm{obs}}-\sigma_i^{\mathrm{model}}(v;\theta_{\mathbb Q})]^\top[\sigma_i^{\mathrm{obs}}-\sigma_i^{\mathrm{model}}(v;\theta_{\mathbb Q})]\), the date-level derivative is
\begin{equation}
\frac{\partial L_i(v)}{\partial v}
=
-2J_i(v;\theta_{\mathbb Q})^\top
\left[\sigma_i^{\mathrm{obs}}-\sigma_i^{\mathrm{model}}(v;\theta_{\mathbb Q})\right].
\label{eq:app_state_loss_derivative}
\end{equation}
A central finite-difference derivative is retained for validation, with one-sided differences at the variance boundaries. The derivative formula is not used for low-vega contracts or when price and implied-volatility clipping makes the inversion locally non-differentiable.

The state-solver benchmark compares golden-section search, bounded Brent minimisation and bounded least-squares procedures using finite-difference and semi-analytical Jacobians. Bounded Brent is used in the production configuration on \(\mathcal V=[10^{-6},0.15]\), with a warm-start window of 0.04 and golden-section search retained as a fallback. The benchmark has zero failures for bounded Brent across the clean panel and the three rounded and capped observation-error designs. Solver name, objective evaluations, boundary hits, Jacobian evaluations and fallback use are recorded. The outer first-step C-GMM criterion remains derivative free.
