function sigma = fast_bs_imp_vol_fwd( obs_price, T, F, k, r)
...
%    k = strike price relative to forward price (e.g., K = k*F)
%    r = risk free rate used to compute the option price
% 
%   Outputs:
%   out = implied volatility of the call option price

sigma = imp_vol_v2_0(F, T, k*F, obs_price*exp(r*T), 'call');
end


function sigma = imp_vol_v2_0(underlying, tau, strike, price, type)
...
implied_volatility_maximum_iterations = 10;
asymptotic_expansion_accuracy_threshold = -20;
