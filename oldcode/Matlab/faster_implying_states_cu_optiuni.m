...
% K_vect(1,1,1) = 1;
K_vect = [1 1.2]; %moneyness (strike to ATM ratio) 
K = 1; 
r = 0;
T = 3/12;
...
% theta  = [3.6105457291809,-2.05211643077148,-0.0146108243499099,0.235139564785552,-0.646143417742387];
tic
ISQMLE(theta,N_timepoints,opt_price,K_vect,T,r,states)
toc

funcmin_handle = @(theta) -ISQMLE(theta,N_timepoints,opt_price,K_vect,T,r,states);
...
function out = ISQMLE(theta,N_timepoints,opt_price,K_vect,T,r,states)
  ...
  parfor i = 1:N_timepoints
   ...
   state2_test(i,1) = lsqnonlin(minfun,.2,[],[],options); 
  end
  % toc
  ...
  for i=2:N_timepoints
    [Mu(1),Mu(2),Sigma(1,1),Sigma(2,2),Sigma(1,2)] = heston_qmle_moments(state2_test(1,1),theta);
    Sigma(2,1)=Sigma(1,2); 
    logpdf(i,1) = logmvnpdf([states(i,1)-states(i-1,1), state2_test(i,1)],Mu,Sigma);
  end
...
if nargout==2
cfv2_fod = exp( coeffs(:,1)*ones(1,length(K_vect)*size(states,1)) + (repmat(coeffs(:,2:end)*states',[1,length(K_vect)]) - 1i*k_grid*pi/(b-a)*(reshape((states(:,1) + log(K_vect)),[1,length(states(:,1))*length(K_vect)])))).*coeffs(:,3); 
