...
for k=2:N_timepoints-1
 for j=1:2:n_particles    
    ...
    particle_PDF(k,j) = exp(-0.5 * (((states(k+1,1)-states(k,1)) - td_mean1)./td_std1).^2) ./ (sqrt(2*pi) .* td_std1);
    td_mean2 = (eta-0.5)*v_particles(k,j+1)*deltat;
    td_std2 = sqrt(deltat)*sqrt(v_particles(k,j+1));
    particle_PDF(k,j+1) = exp(-0.5 * (((states(k+1,1)-states(k,1)) - td_mean2)./td_std2).^2) ./ (sqrt(2*pi) .* td_std2);
 end
