...
rho = -0.5; 
eta_v = 0;
theta_V = [kappa, vbar, sigmav, rho, eta_v];
eta = 2;
theta_S = eta;
...
% K_vect(1,1,1) = 1;
K_vect = [.8 1 1.2]; %moneyness (strike to ATM ratio) 
K = 1; 
r = 0;
T = [1/12 6/12];
...
[opt_price, opt_price_jacob, coeffs] = opt_pricing(K_vect,T,r,theta,states);
opt_price = reshape(opt_price,[N_timepoints,length(K_vect),length(T)]);
toc
...
% out = fmincon(funcmin_handle,theta,[],[],[],[],[1e-5 1e-5, 1e-5, 1e-5, -1, -5 ],[inf, inf, inf, .5, 1, 5],[],options);
...
%   implying latent vol from option prices
  ...
  for i = 1:N_timepoints
   ...
   state2_test(i,2) = max([lsqnonlin(minfun,.2,[],[],options),1e-20]); 
  end
%   toc
...
%    means = (theta(1)-0.5)*state2_test(i-1,2)*1/252;
%    stdevs = sqrt(1/252)*sqrt(state2_test(i-1,2));
%    mockup = stdevs.^2;
%    logpdf(i,1) = exp(-0.5 * (((states(i,1)-states(i-1,1)) - means)./stdevs).^2)./(sqrt(2*pi).*stdevs);
  end
  out = sum(logpdf)./(N_timepoints-1);
 fprintf('theta: %d %d %d %d %d %d; crit: %d \n',[theta out]);
  % toc
end
...
cfv2 = exp( coeffs(:,1,tt)*ones(1,length(K_vect)*size(states,1)) + (repmat(coeffs(:,2:end,tt)*states',[1,length(K_vect)]) - 1i*k_grid*pi/(b-a)*(reshape((states(:,1) + log(K_vect)),[1,length(states(:,1))*length(K_vect)])))); 
out(:,:,tt) = exp(-r*T(tt)) * exp(reshape((states(:,1) + log(K_vect)),[1,length(states(:,1))*length(K_vect)])) .* real(sum(bsxfun(@times,[.5;ones(N_cos-1,1)].*exp(-1i*(k_grid*a*pi/(b-a))).*V_call,cfv2)));
end
%Attempt to compute the first order derivative analytically
if nargout==2
for tt=1:length(T)
  cfv2_fod = exp( coeffs(:,1,tt)*ones(1,length(K_vect)*size(states,1)) + (repmat(coeffs(:,2:end,tt)*states',[1,length(K_vect)]) - 1i*k_grid*pi/(b-a)*(reshape((states(:,1) + log(K_vect)),[1,length(states(:,1))*length(K_vect)])))).*coeffs(:,3,tt); 
  out2(:,:,tt) = exp(-r*T(tt)) * exp(reshape((states(:,1) + log(K_vect)),[1,length(states(:,1))*length(K_vect)])) .* real(sum(bsxfun(@times,[.5;ones(N_cos-1,1)].*exp(-1i*(k_grid*a*pi/(b-a))).*V_call,cfv2_fod)));
end
varargout{1} = out2;
elseif nargout==3
varargout{2} = coeffs;
