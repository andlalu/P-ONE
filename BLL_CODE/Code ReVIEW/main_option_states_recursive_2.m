...
[deriv_data, states, TT_marks, coeff_mat ] = sim_deriv_data([log(100),0.2^2,5],N,dt,type,theta_SV,814724);%640449
...
norm2=10;
epsilon=1e-3;
theta_SV2=theta_SV;%.*(0.5+rand(size(theta_SV)));
out2(1,:)=theta_SV2
%     lb=[ 0.5,   2, 0.005, 0.1, -0.8];
%     ub=[ 5.0,  10, 0.100, 0.4, -0.3];
alpha=1e-20;
i=2
while norm2>epsilon
states_imp2=impl_state_func_fut(deriv_data, TT_marks, type, theta_SV2);
...
% func3 = @(freetheta) func2(freetobound(type,freetheta,lb,ub));
func2(theta_SV)
options=optimset('Display','iter');
% aux=fminsearch(func3,boundtofree(type,theta_SV2,lb,ub),options)
out2(i,:)=fminsearch(func2,theta_SV2,options)%freetobound(type,aux,lb,ub)
norm2=norm(theta_SV2-out2(i,:))
theta_SV2=out2(i,:);
i=i+1
end
toc
