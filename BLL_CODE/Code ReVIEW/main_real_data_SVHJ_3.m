...
N=723;
aux=impl_state_func_fut(data(data(:,1)<=723,:), TT_marks, type, theta_SVHJ3.*[ones(1,9) c c]).*repmat([1 1 1./c],[N-1,1]);
...
states_imp(:,:,ratios==rate)=impl_state_func_fut(data(data(:,1)<=723,:), TT_marks, type, theta_SVHJ3.*[ones(1,9) c c]).*repmat([1 1 1./c],[N-1,1]);
out1=[out1; critstep1(theta_SVHJ3, states_imp(:,:,ratios==rate), dt, type)];
...
legend({'2nd step Criterion' 'Function Values'})
h=xlabel('$\delta$');
set(h,'Interpreter','latex')
% subplot(2,2,2)
% plot(squeeze(states_imp(:,2,:))-repmat(squeeze(states_imp(:,2,5)),[1,9]))%,[1,length(ratios)])
% legend('Implied vols (v[alternative]-v[estimate])')
% subplot(2,2,4)
% plot(squeeze(states_imp(:,3,:).*c)-repmat(squeeze(states_imp(:,3,5)).*c,[1,9]))%,[1,length(ratios)])
% legend('Implied intens (\lambda[alternative]-\lambda[estimate])')
