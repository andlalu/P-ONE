...
%     out1=[out1; critstep11(theta_SVHJ2, states_estim, dt, type)];
% %     out2=[out2; critstep2(theta_SVHJ2, states_estim, dt, type, H0, OptMatrix)];
% end
% subplot(1,2,1)
...
theta_SVHJ2(11)=22./c;
% [OptMatrix, H0]=funcH0(theta_SVHJ2, states_estim, dt,type, alpha);
lb=[-.4, -0.40, 0.02, 0.5,   2, 0.005, 0.1, -0.8, 1.45, 0.05./c, 3./c]; %bound for theta(1) and theta(9) are here bounds for the difference
ub=[0,  -0.04, 0.20, 5.0,  10, 0.10, 0.4, -0.3, 30, 1.50./c, 30./c];
func1 = @(theta) critstep11(freetobound(type,theta,lb,ub),states_estim,dt,type);
func1(boundtofree(type,theta_SVHJ3,lb,ub))
options=optimset('Display','iter','MaxIter',250,'TolFun',1e-3);%,'TolX',1e-10,);
k2=fminunc(func1,boundtofree(type,theta_SVHJ2,lb,ub),options);
theta_SVHJ3
freetobound(type,k2,lb,ub)
% k1
