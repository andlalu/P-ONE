%%%% 01.06.2015 V3.0 Beta for SVHJ - FINAL SET-UP!!!
%% Intro
clc;clear;
...
type='SVHJ';
c=100;
states = sim_states([log(100),0.2^2,delta0],N,dt,type,theta_SVHJ);%491270);%,871045);%394068); %394068Exemplu de seed foarte bun la 501 instante 776528
states_estim = [diff(states(:,1)),states(2:end,2),states(2:end,3)./c]; %create stationary return series for estimation
alpha=1e-1;
theta_SVHJ3=theta_SVHJ.*1;
% theta_SVHJ3(9)=theta_SVHJ(9);
theta_SVHJ3(10)=theta_SVHJ(10)./c;
theta_SVHJ3(11)=theta_SVHJ(11)./c;
theta_SVHJ2=theta_SVHJ3;
[OptMatrix, H0 ]=funcH0(theta_SVHJ2,states_estim,dt,type,1e-18);
out1=[]
out2=[]
time=[];
ratios=0.6:.1:1.4;
...
j=1;
for rate2=ratios
    theta_SVHJ2(vartest2)=theta_SVHJ3(vartest2).*rate2;
    for rate=ratios
    time1=tic;
    i
    j
    theta_SVHJ2(vartest)=theta_SVHJ3(vartest).*rate;
    if(theta_SVHJ2(9)>(theta_SVHJ2(11).*c)+1)
%     out1(i,j)=critstep11(theta_SVHJ2, states_estim, dt, type);
    aux_out1(i,j)=theta_SVHJ2(vartest);
    aux_out2(i,j)=theta_SVHJ2(vartest2);
% % % %     [OptMatrix, H0 ]=funcH0(theta_SVHJ2,states_estim,dt,type,1e-18);
    out1(i,j)=critstep2(theta_SVHJ2, states_estim, dt, type, H0, OptMatrix);
    end
    clc;
    time=[time,toc(time1)];
    fprintf('time left aprox:%2.1f minutes\n',(length(ratios)^2-i*j).*mean(time)/60)
    j=j+1;
    end
    j=1;
    i=i+1;
...
out1(out1==0)=NaN;
disp('k');
aux_out1(out1==(min(min(out1))))
disp('d');
aux_out2(out1==(min(min(out1))))
% subplot(1,2,1)
...
% %Minimization SVHJ with True State
% theta_SVHJ2=theta_SVHJ3;
% % [OptMatrix, H0]=funcH0(theta_SVHJ2, states_estim, dt,type, alpha);
% func1 = @(theta) critstep1(theta,states_estim,dt,type);
% func1(theta_SVHJ3)
% options=optimset('Display','iter','MaxIter',150,'TolFun',1e-3);%,'TolX',1e-10,);
% k2=fminsearch(func1,theta_SVHJ2,options);
% theta_SVHJ3
% k2
% % k1
...
% % % % % % % % states = sim_states([log(100),0.2^2,5],N,dt,type,theta_SV,640449);%394068); %394068Exemplu de seed foarte bun la 501 instante 776528
% % % % % % % % states_estim = [diff(states(:,1)),states(2:end,2)]; %create stationary return series for estimation
% % % % % % % % alpha=1e-20;
% % % % % % % % out1=[]
% % % % % % % % out2=[]
% % % % % % % % out12=[]
% % % % % % % % theta_SV2=[3.14328830986134,6.28657661972268,0.0130970346244222,0.288134761737289,-0.785822077465335];%theta_SV*(0.5+rand(1,1));
% % % % % % % % [OptMatrix, H0]=funcH0(theta_SV2, states_estim, dt, type, alpha);
% % % % % % % % ratios=0.1:.5:2.6;
% % % % % % % % i=1;j=1;
% % % % % % % % for rate=ratios
% % % % % % % %     theta_SV2(1)=theta_SV(1).*rate;
% % % % % % % %     for rate2=ratios
% % % % % % % %         theta_SV2(2)=theta_SV(2).*rate2;
% % % % % % % %         out1(i,j)=critstep1(theta_SV2, states_estim, dt, type);
% % % % % % % %         out12=[out12; critstep2(theta_SV2, states_estim, dt, type, H0, eye)];
% % % % % % % %         out2(i,j)=critstep2(theta_SV2, states_estim, dt, type, H0, OptMatrix);
% % % % % % % %         j=j+1
% % % % % % % %     end
% % % % % % % %     j=1;
% % % % % % % %     i=i+1
