...
tic
[deriv_data, states, TT_marks, coeff_mat ] = sim_deriv_data([log(100),0.2^2,5],N,dt,type,theta_SVHJ,seed);
%Imply states back from prices assuming correct parameter vector:
disp('pricing options took') 
toc
%Temporary
% tic
% [states_imp,x_imp]=impl_state_func_fut(deriv_data, TT_marks, type, theta_SV, coeff_mat);
% theta_SV2=theta_SV;

% toc

% theta_SV2=theta_SV;%*(0.5+rand(1,1));%theta_SV;%[3.14328830986134,6.28657661972268,0.0130970346244222,0.288134761737289,-0.785822077465335];%theta_SV*(0.5+rand(1,1));
% theta_SV2(4)=theta_SV(4).*0.8;
%theta_SV2=[1.5 3 0.018 0.25 -0.4];
tic
[states_imp2,x_imp2]=impl_state_func_fut(deriv_data, TT_marks, type, theta_SVHJ);
disp('backing out states took')
toc
% states_estim = [diff(states(:,1)),states_imp2(2:end,2)]; %create stationary return series for estimation
% [OptMatrix1, H01]=funcH0(theta_SV2, states_estim, dt, type, alpha);
% ratios=0.6:.1:1.4;
% vartest=2;
% i=1;
% for rate=ratios
%     theta_SV2(vartest)=theta_SV(vartest).*rate;
%     rate
%         states_imp3(:,:,i)=impl_state_func_fut(deriv_data, TT_marks, type, theta_SV2);
%         states_estim = [diff(states(:,1)),states_imp3(2:end,2)]; %create stationary return series for estimation
...
%         comp3 = [comp3, critstep2(theta_SV2, states_estim, dt, type, H01, OptMatrix1)];
%         i=i+1;
% end
% aux=1.*(ratios==1);aux(aux==0)=NaN;
% subplot(1,4,1)
% plot(ratios.*theta_SV(vartest),comp1,ratios.*theta_SV(vartest),aux.*min(comp1),'*k')
% legend('1st step')
% subplot(1,4,2)
% plot(ratios.*theta_SV(vartest),comp2,'r',ratios.*theta_SV(vartest),aux.*min(comp2),'*k')
% legend('2nd step')
% subplot(1,4,3)
% plot(ratios.*theta_SV(vartest),comp3,'r',ratios.*theta_SV(vartest),aux.*min(comp3),'*k')
% legend('2nd step-1optim')
% subplot(1,4,4)
% plot(repmat(states(:,2),[1,length(ratios)])-squeeze(states_imp3(:,2,:)))
% legend('Imp State Differences');
