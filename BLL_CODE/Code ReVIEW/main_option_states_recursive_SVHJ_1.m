...
[deriv_data, states, TT_marks, coeff_mat ] = sim_deriv_data([log(100),0.2^2,5],N,dt,type,theta_SVHJ,seed);%640449
...
% theta_SVHJ2=lb+(rand(size(theta_SVHJ))).*(ub-lb);
% theta_SVHJ2(9) = theta_SVHJ2(11) + theta_SVHJ2(9); %theta(9) stores the difference, theta(11) the level
% theta_SVHJ2(1) = theta_SVHJ2(2) + theta_SVHJ2(1);
% theta_SVHJ2=[-0.1   -0.2    0.1    1    6    0.015    0.15   -0.5   25   0.3   10];
% out2(1,:)=theta_SVHJ2
% alpha=0.02;
% i=2;
% while norm2>epsilon
% states_imp2=impl_state_func_fut(deriv_data, TT_marks, type, theta_SVHJ2);
% states_estim = [diff(states(:,1)),states_imp2(2:end,2:end)]; %create stationary return series for estimation
...
% out2(i,:)=fminsearch(func2,theta_SVHJ2,options)
...
comp1=[]
comp2=[]
% % % % theta_SV2=theta_SV;%[3.14328830986134,6.28657661972268,0.0130970346244222,0.288134761737289,-0.785822077465335];%theta_SV*(0.5+rand(1,1));
...
for rate=ratios
    ...
    rate
        states_imp(:,:,jj)=impl_state_func_fut(deriv_data, TT_marks, type, theta_SVHJ2);
        states_estim = [diff(states(:,1)),states_imp(1:end,2,jj),states_imp(1:end,3,jj)./c]; %create stationary return series for estimation
        theta_SVHJ3=theta_SVHJ2;
        theta_SVHJ3(10)=theta_SVHJ2(10)./c;
        theta_SVHJ3(11)=theta_SVHJ2(11)./c;
        [OptMatrix, H0]=funcH0(theta_SVHJ2, states_estim, dt, type, alpha);
        comp1 = [comp1, critstep1(theta_SVHJ3, states_estim, dt, type)];
        comp2 = [comp2, critstep2(theta_SVHJ3, states_estim, dt, type, H0, OptMatrix)];
        jj=jj+1;
end
