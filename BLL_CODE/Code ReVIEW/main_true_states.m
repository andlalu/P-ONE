...
% for i=1:nmc
states = sim_states([log(100),0.2^2,5],N,dt,type,theta_SV,1111);%,394068); %394068Exemplu de seed foarte bun la 501 instante 776528
states_estim = [diff(states(:,1)),states(2:end,2)]; %create stationary return series for estimation
% 
% alpha=1e-20;
% 
theta_SV3=theta_SV.*1;
theta_SV2=theta_SV3;
out1=[]
out2=[]
time=[];
...
j=1;
for rate2=ratios
    theta_SV2(vartest2)=theta_SV3(vartest2).*rate2;
    for rate=ratios
    time1=tic;
    i
    j
    theta_SV2(vartest)=theta_SV3(vartest).*rate;
% % % %     if(theta_SV2(9)>(theta_SV2(11).*c)+1)
%     out1(i,j)=critstep11(theta_SV2, states_estim, dt, type);
    aux_out1(i,j)=theta_SV2(vartest);
    aux_out2(i,j)=theta_SV2(vartest2);
    [OptMatrix, H0 ]=funcH0(theta_SV2,states_estim,dt,type,1e-20);
    out1(i,j)=critstep2(theta_SV2, states_estim, dt, type, H0, OptMatrix);
    clc
    time=[time,toc(time1)];
...
% % theta_SV3=out2(end,:);
% ratios=0.5:.1:2;
% [OptMatrix, H0]=funcH0(theta_SV3,states_estim, dt, type, alpha);
% for rate=ratios
%     theta_SV3(vartest)=theta_SV(vartest).*rate;
%         comp1 = [comp1, critstep11(theta_SV3, states_estim, dt, type)];
