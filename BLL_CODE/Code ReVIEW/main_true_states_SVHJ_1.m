...
type='SVHJ';
c=10;
states = sim_states([log(100),0.2^2,5],N,dt,type,theta_SVHJ);%,491270);%,491270);%,871045);%394068); %394068Exemplu de seed foarte bun la 501 instante 776528
states_estim = [diff(states(:,1)),states(2:end,2),states(2:end,3)./c]; %create stationary return series for estimation
alpha=0.02;
theta_SVHJ3=theta_SVHJ;
theta_SVHJ3(9)=theta_SVHJ(9);
theta_SVHJ3(10)=theta_SVHJ(10)./c;
theta_SVHJ3(11)=theta_SVHJ(11)./c;
theta_SVHJ2=theta_SVHJ3;
% theta_SVHJ2=[-0.0639 -0.1299 0.0602 3.0995 4.4519 0.01 0.2681 -0.6946 18 0.0031 0.1490];
...
for rate=ratios
    ...
    out1=[out1; critstep1(theta_SVHJ2, states_estim, dt, type)];
    out2=[out2; critstep2(theta_SVHJ2, states_estim, dt, type, H0, OptMatrix)];
end
subplot(1,2,1)
aux=1.*(ratios==1);aux(aux==0)=NaN;
plot(ratios.*theta_SVHJ3(vartest),out1,ratios.*theta_SVHJ3(vartest),aux.*min(out1),'*k')
legend('1st step')
subplot(1,2,2)
