...
namespace {
  ...
  // the hardware (or compiler implementation) accuracy of exp(-x_sqr/2) which is not reliably more than 14 digits when
...
namespace {
  ...
  const double const1 = 0.180625; // = split1_sqr
...
double erfinv(double e) {
  // Phi(x) = erfc(-x/sqrt2)/2 = (1+erf(x/sqrt2))/2 = erf(x/sqrt2)/2 + half.
  // erf(z) = 2 * ( Phi(sqrt2*z) - half ).
  // Hence, if e = erfc(z), and y is the solution to Phi(y) - half = e/2, then z = y/sqrt2.
  const double q = 0.5 * e;
  if (fabs(q) <= split1) {
    const double r = const1 - q * q;
    return q * AB(r) * (1 / SQRT_TWO);
  }
  double r = -0.5 * fabs(e) + 0.5;
  r = sqrt(-log(r));
  double ret;
  if (r < split2) {
    r = r - const2;
...
//
// ======================================================================================
// Copyright (c) 2024 Peter Jaeckel.
//
...
inline
#endif
// Specialisation of x = Phi_minus_1(p) for x <= -1, p = Phi(x) <= 0.1586552539314570514148
double inverse_norm_cdf_for_low_probabilities(double p) {
  ...
  //                    |          I.       |       II.       |      III.      |      IV.      |        V.          |
  //                    |         <--->       |       <--->       |      <--->       |      <--->      |       <--->          |
  //  In r=sqrt|ln(p)|:    [ 1.35684  ,      2.05       ,      3.41      ,       6.7      ,     12.9       ,    27.314 ]
  //  In |x|:           [    1     ,     2.1712      ,     4.2905     ,     9.1375     ,    18.0331     ,    38.509 ]
  //  In p = Phi(-|x|):   [ 0.15866  ,    0.014958     ,    8.912E-6    ,    3.1954E-20  ,   5.35865E-73  ,    1E-324 ]   ==>  Compare this with DBL_TRUE_MIN = 4.94E-324.
...
namespace {
  // u_m_a_x = Phi(1) - half = 0.3413447460685429
  const double u_m_a_x = 0.3413447460685429;
}

// Inverse of Phi(x)-half, i.e., for a given u in [-u_m_a_x,u_m_a_x] find x such that u = Phi(x)-half.
inline double inverse_norm_cdfmhalf_for_midrange_probabilities(double u) {
  assert(!(fabs(u) > u_m_a_x)); // This formulation permits NaN to pass through below. The result will be NaN as it should.
  // innermost branch: x in [-1,1], since u = Phi(x)-half => u in [-u_m_a_x,u_m_a_x] with u_m_a_x=Phi(1)-half=0.3413447460685429, and p in [0.15866,0.84134].
    // Define f(x) := sqrt(2pi)*(Phi(x)-half). It is an odd function: f(-x) = -f(x). Its Taylor expansion is f(x) = x - x_cube/6 + x_5/40 - O(x_7).
    // Set y = f(x). The inverse function f_minus_1(y) is also odd with its Taylor expansion as f_minus_1(y) = y + y_cube/6 + 7*y_5/120 + O(y_7).
    // Define gtilde(y) := (f_minus_1(y)/y-1)/y_sqr which is now an even function and we can focus on y >= 0. We define q := y_sqr and g(q) := gtilde(sqrtq) = (f_minus_1(sqrtq)/sqrtq-1)/q.
    // We then compute a Remez-optimized minimax rational function approximation of order (5,5) for g(q).
    // In the end, we transform to h(s) := sqrt(2pi)*(1+2pi*(u_m_a_x_sqr-s)*g(2pi*(u_m_a_x_sqr-s)))   with   s = u_m_a_x_sqr - u_sqr  where u_m_a_x = f(x_m_a_x)/sqrt(2pi) = (Phi(x_m_a_x)-half).  Note that this makes h(s) a (6,5) rational function.
    // Accuracy better than 9.8E-17 in perfect arithmetic within this branch.
  const double s = u_m_a_x * u_m_a_x - u * u;
...
// Inverse of Phi(x). Given p, return x such that p = Phi(x), i.e., x = Phi_minus_1(p).
double inverse_norm_cdf(double p) {
  const double u = p - 0.5;
  if (fabs(u) < u_m_a_x)
      return inverse_norm_cdfmhalf_for_midrange_probabilities(u);
  // Tail probability min(p,1-p) = 0.15866. r = sqrt(-ln(min(p,1-p))) > 1.3568425277.
...
double erfinv(double e) {
  ...
  // Hence, if e = erfc(z), and y is the solution to Phi(y) - half = e/2, then z = y/sqrt2.
  if (fabs(e) < (2 * u_m_a_x))
    return inverse_norm_cdfmhalf_for_midrange_probabilities(0.5 * e) * (1 / SQRT_TWO);
  return (e < 0 ? inverse_norm_cdf_for_low_probabilities(0.5 * e + 0.5) : -inverse_norm_cdf_for_low_probabilities(-0.5 * e + 0.5)) * (1 / SQRT_TWO);
}
